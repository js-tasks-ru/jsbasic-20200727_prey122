...
export default class RibbonMenu {
  ...
  render(arr){
    let ribbon_menu = arr.map(value => `<a href="#" class="ribbon__item" data-id="${value.id}">${value.name}</a>`).join('');
    this.elem.innerHTML ='<nav class="ribbon__inner">'+ ribbon_menu +'</nav>';
  ...
  onClick(event){
    ...
    if(event.target.closest('.ribbon__item')){
      ...
      if(this.item_active){
        this.item_active.classList.remove('ribbon__item_active');
      }
      
      event.target.classList.add('ribbon__item_active');
      this.item_active = event.target;
      let data_set = event.target.dataset.id;
      if(data_set == ''){
        data_set ='All';
      }
      let custom_event = new CustomEvent('ribbon-select',{
        detail : data_set,
        bubbles: true
      })
      this.elem.dispatchEvent(custom_event);
    }
  }
}
